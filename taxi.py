...
class Agent():
    ...
    def choose_action(self, state):
        ...
        if random.uniform(0,1) < self.epsilon: # explore mode
            ...
            action = np.argmax(self.qtable[state]) # select action with max q value
        return action
        # End your code
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        ori_Qvalue = float()
        new_Qvalue = float()
        ori_Qvalue = self.qtable[state][action] # original Q value with state and action
        nextmax = np.max(self.qtable[next_state]) # max Q value of one of action in qtable with next_state

        # Q-learning : qtable updating formula as below.
        new_Qvalue = (1-self.learning_rate) * ori_Qvalue + self.learning_rate * (reward + self.gamma * nextmax)
    
        self.qtable[state][action] = new_Qvalue # update qtable with new Q value
        # End your code
