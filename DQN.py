...
class Agent():
    ...
    def learn(self):
        ...
        # Begin your code
        # TODO
        # use buffer.sample to sample a tuple of information, and use tensor to get the right type
        ...
        # eval: Qvalue from evaluate net for the current state
        # next: get the values for next_state, we want to use the max of these values later for "target"
        # target: Qvalue from target net, will be updated by evaluate net every 100 times
        #         target <- reward + discount * nextmax *(~done)
        eval = self.evaluate_net(state).gather(1, action.reshape(self.batch_size,1))
        next = self.target_net(next_state).detach()
        target = reward.reshape(self.batch_size,1) + self.gamma * next.max(1).values.unsqueeze(1) * (~done).reshape(self.batch_size,1)

        # doing step 4.~7.
        loss_func = nn.MSELoss()
        loss = loss_func(eval,target) # compute loss
    ...
    def choose_action(self, state):
        ...
        with torch.no_grad():
            ...
            if random.uniform(0,1) < self.epsilon: # explore mode
                action = self.env.action_space.sample()
            else:
                evaluate_Qvalue = self.evaluate_net.forward(torch.FloatTensor(state)) # select action with max q value
                action = torch.argmax(evaluate_Qvalue).item()
            # End your code
        return action
    ...
    def check_max_Q(self):
        ...
        # Begin your code
        # TODO
        state = torch.FloatTensor(self.env.reset()) # get initial state by env.reset()
        return torch.max(self.target_net(state)) # return max Q value for the state in target network
        # End your code
