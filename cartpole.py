...
class Agent():
    ...
    def discretize_observation(self, observation):
        ...
        # Begin your code
        # TODO
        # divide 4 observations and append into a state list
        state = list()
        for i in range(4):
    ...
    def choose_action(self, state):
        ...
        if random.uniform(0,1) < self.epsilon: # explore mode
            ...
            action = np.argmax(self.qtable[tuple(state)]) # select action with max q value
        return action
        # End your code
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        # Begin your code
        # TODO
        # mostly same as in taxi.py

        ori_value = self.qtable[tuple(state)][action] # original Q value with tuple of states and action
        nextmax = np.max(self.qtable[tuple(next_state)]) # max Q value of one of action in qtable with tuple of next_state

        # Q-learning : qtable updating formula as below.
        new_value = (1-self.learning_rate) * ori_value + self.learning_rate * (reward + self.gamma * nextmax)

        self.qtable[tuple(state)][action] = new_value # update qtable with new Q value
        # End your code
    ...
    def check_max_Q(self):
        ...
        # Begin your code
        # TODO
        state = self.discretize_observation(env.reset()) # get initial state by env.reset()
        return np.max(self.qtable[tuple(state)]) # return max Q value for the state
        # End your code
